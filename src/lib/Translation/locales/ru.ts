export const ru = {
    ...
    Welcome_Message: `
        ...
        • **Массовая Рассылка**: "отправить письмо в #название-канала или #название-команды" *(требует права администратора или специальные разрешения)*
        ...
        🔒 **Безопасность Массовой Рассылки:**
        Только администраторы рабочего пространства и специально авторизованные пользователи могут отправлять массовые письма в каналы/команды. Это предотвращает спам и обеспечивает ответственное использование электронной почты.
    ...
    Stats_Command: "используйте `/email stats <количество дней>` - Получить отчет статистики электронной почты (максимум 15 дней)",
    Natural_Language_Examples: "используйте `/email <ваш запрос>` - Команды на естественном языке для помощи ИИ\nПримеры:\n• \`/email отправить письмо @john.doe о встрече завтра\`\n• \`/email подытожить эту ветку и отправить manager@company.com\`\n• \`/email сгенерировать статистику за последние 5 дней\`\n• \`/email отправить письмо @all OR #название-канала OR #название-команды\` *(требует права администратора)*\n\n🔒 **Массовая Рассылка**: Письма в каналы/команды требуют права администратора рабочего пространства или специальные разрешения для предотвращения спама.",
    ...
    Email_Categorization_Label: "Метод Категоризации Электронной Почты",
    Email_Categorization_Description: "Выберите, как следует категоризировать письма для статистики",
    Email_Categorization_Email_Provider: "API Провайдера Электронной Почты",
    Email_Categorization_LLM: "LLM Анализ",
    ...
    Bulk_Email_Permission_Denied: "🚫 **Доступ запрещен: Функция массовой рассылки**\n\nТолько администраторы рабочего пространства и специально авторизованные пользователи могут отправлять массовые письма.\n\n**Авторизованные пользователи:**\n• **Администраторы рабочего пространства** - Полный доступ ко всем функциям\n• **Одобренные пользователи** - Пользователи, которым администраторы специально предоставили разрешения\n\n**Что вы можете сделать?**\n• Обратитесь к администратору рабочего пространства для запроса разрешений на массовую рассылку\n• Попросите администратора добавить ваше имя пользователя в список разрешенных пользователей\n\nНужна помощь? Обратитесь к администратору рабочего пространства за поддержкой.",
    Bulk_Email_Permission_Check_Error: "❌ **Ошибка проверки разрешений**\n\nПроизошла ошибка при проверке ваших разрешений для функций массовой рассылки. Попробуйте снова или обратитесь к администратору, если проблема сохраняется.",
